				...
				<form id="form1" method="post" action="adminNoticeUpdate">
					<div id="container">
						<div class="info">
							...
							<p>
								작성자: <input type="text" name="writer" value="관리자">
								<select name="top_state">
									<c:choose>
										<c:when test="${item.top_state eq 'y'}">
											<option value="y" selected>공지게시</option>
											<option value="n">일반게시</option>
										</c:when>
										<c:otherwise>
											<option value="n" selected>일반게시</option>
											<option value="y">공지게시</option>
										</c:otherwise>
									</c:choose>
								</select>
							</p>
