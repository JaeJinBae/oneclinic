	...
	$(function(){
      ...
      //익스플로러에서 한글 검색 후 read로 넘어갈 때 인코딩 문제 아래와 같이 해결
		$(".title > a").click(function(e){
			...
			var k=$("input[name='keyword']").val();
    		var keyword = encodeURIComponent(k);
    		location.href="${pageContext.request.contextPath}/admin/adminAdviceRead${pageMaker.makeQuery(pageMaker.cri.page)}&searchType="+searchType+"&keyword="+keyword+"&no="+no;
		});
	});
</script>
</head>
						    ...
						    <c:otherwise>
						        <c:forEach var="item" items="${list}">
									<tr>
										...
										<td class="title"><a href="${pageContext.request.contextPath}/admin/adminAdviceRead${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}">${item.title}</a></td>
										<td>${item.writer}</td>
										<td>${item.regdate}</td>
										<td>${item.cnt}</td>
										<td>
